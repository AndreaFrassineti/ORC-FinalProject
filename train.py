...
def train_model(dataset_path, model_save_path, epochs=100):
    # check existance file
    if not os.path.exists(dataset_path):
        print(f"Dataset not found: {dataset_path} -> skipping this task.")
        return
    ...
    print(f"    'Feasible' points (1): {n_ones} ({n_ones/len(states):.1%})")

    # Dataset and Split (80% Train, 20% Test)
...
def train_everything():
    ...
    for i, task in enumerate(tasks):
        print(f"\n🔹 TASK {i+1}/{len(tasks)}")
        train_model(task["dataset"], task["model"], task["epochs"])
